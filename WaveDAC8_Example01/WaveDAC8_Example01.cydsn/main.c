...
int main()
{
    ...
    for(;;){         /* Loop forever    */
        ...
        for (int i=0; i<107; i++){
            if ((freq == intFrequency[i]) && playing){
                noteIndex = i;
                sprintf(transmitBuffer, "%c%c%c : %i \n\r", notes[noteIndex][0],notes[noteIndex][1], notes[noteIndex][2], intFrequency[noteIndex]);

/*******************************************************************************
* File Name: analogADC_X_theACLK.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: analogADC_X_theACLK_Start
********************************************************************************
*
* Summary:
...
void analogADC_X_theACLK_Start(void) 
{
    ...
    analogADC_X_theACLK_CLKEN |= analogADC_X_theACLK_CLKEN_MASK;
	analogADC_X_theACLK_CLKSTBY |= analogADC_X_theACLK_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: analogADC_X_theACLK_Stop
********************************************************************************
*
* Summary:
...
void analogADC_X_theACLK_Stop(void) 
{
    ...
    analogADC_X_theACLK_CLKEN &= (uint8)(~analogADC_X_theACLK_CLKEN_MASK);
	analogADC_X_theACLK_CLKSTBY &= (uint8)(~analogADC_X_theACLK_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: analogADC_X_theACLK_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((analogADC_X_theACLK_CLKEN & analogADC_X_theACLK_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = analogADC_X_theACLK_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = analogADC_X_theACLK_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* analogADC_X_theACLK__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(analogADC_X_theACLK_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(analogADC_X_theACLK_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: analogADC_X_theACLK_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: analogADC_X_theACLK_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = analogADC_X_theACLK_GetSourceRegister();
    uint16 oldDivider = analogADC_X_theACLK_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = analogADC_X_theACLK_CLKEN & analogADC_X_theACLK_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(analogADC_X_theACLK_DIV_PTR, clkDivider);
                analogADC_X_theACLK_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                analogADC_X_theACLK_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(analogADC_X_theACLK_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((analogADC_X_theACLK_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(analogADC_X_theACLK_DIV_PTR, clkDivider);
				analogADC_X_theACLK_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: analogADC_X_theACLK_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(analogADC_X_theACLK_DIV_PTR);
}


/*******************************************************************************
* Function Name: analogADC_X_theACLK_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    analogADC_X_theACLK_MOD_SRC |= modeBitMask & (uint8)analogADC_X_theACLK_MODE_MASK;
}


/*******************************************************************************
* Function Name: analogADC_X_theACLK_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    analogADC_X_theACLK_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(analogADC_X_theACLK_MODE_MASK));
}


/*******************************************************************************
* Function Name: analogADC_X_theACLK_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return analogADC_X_theACLK_MOD_SRC & (uint8)(analogADC_X_theACLK_MODE_MASK);
}


/*******************************************************************************
* Function Name: analogADC_X_theACLK_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = analogADC_X_theACLK_GetDividerRegister();
    uint8 oldSrc = analogADC_X_theACLK_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        analogADC_X_theACLK_MOD_SRC |= CYCLK_SSS;
        analogADC_X_theACLK_MOD_SRC =
            (analogADC_X_theACLK_MOD_SRC & (uint8)(~analogADC_X_theACLK_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: analogADC_X_theACLK_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return analogADC_X_theACLK_MOD_SRC & analogADC_X_theACLK_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: analogADC_X_theACLK_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    analogADC_X_theACLK_PHASE = clkPhase & analogADC_X_theACLK_PHASE_MASK;
}


/*******************************************************************************
* Function Name: analogADC_X_theACLK_GetPhase
********************************************************************************
*
* Summary:
...
{
    return analogADC_X_theACLK_PHASE & analogADC_X_theACLK_PHASE_MASK;
}

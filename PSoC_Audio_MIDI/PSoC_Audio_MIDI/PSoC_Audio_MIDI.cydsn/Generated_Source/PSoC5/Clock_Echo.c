...
/*******************************************************************************
* Function Name: Clock_Echo_Start
********************************************************************************
*
* Summary:
...
void Clock_Echo_Start(void) 
{
    ...
    Clock_Echo_CLKEN |= Clock_Echo_CLKEN_MASK;
	Clock_Echo_CLKSTBY |= Clock_Echo_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: Clock_Echo_Stop
********************************************************************************
*
* Summary:
...
void Clock_Echo_Stop(void) 
{
    ...
    Clock_Echo_CLKEN &= (uint8)(~Clock_Echo_CLKEN_MASK);
	Clock_Echo_CLKSTBY &= (uint8)(~Clock_Echo_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: Clock_Echo_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((Clock_Echo_CLKEN & Clock_Echo_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = Clock_Echo_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = Clock_Echo_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* Clock_Echo__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(Clock_Echo_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(Clock_Echo_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: Clock_Echo_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: Clock_Echo_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = Clock_Echo_GetSourceRegister();
    uint16 oldDivider = Clock_Echo_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = Clock_Echo_CLKEN & Clock_Echo_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((Clock_Echo_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: Clock_Echo_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(Clock_Echo_DIV_PTR);
}


/*******************************************************************************
* Function Name: Clock_Echo_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Echo_MOD_SRC |= modeBitMask & (uint8)Clock_Echo_MODE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Echo_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Echo_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(Clock_Echo_MODE_MASK));
}


/*******************************************************************************
* Function Name: Clock_Echo_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Echo_MOD_SRC & (uint8)(Clock_Echo_MODE_MASK);
}


/*******************************************************************************
* Function Name: Clock_Echo_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = Clock_Echo_GetDividerRegister();
    uint8 oldSrc = Clock_Echo_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        Clock_Echo_MOD_SRC |= CYCLK_SSS;
        Clock_Echo_MOD_SRC =
            (Clock_Echo_MOD_SRC & (uint8)(~Clock_Echo_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: Clock_Echo_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return Clock_Echo_MOD_SRC & Clock_Echo_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: Clock_Echo_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    Clock_Echo_PHASE = clkPhase & Clock_Echo_PHASE_MASK;
}


/*******************************************************************************
* Function Name: Clock_Echo_GetPhase
********************************************************************************
*
* Summary:
...
{
    return Clock_Echo_PHASE & Clock_Echo_PHASE_MASK;
}

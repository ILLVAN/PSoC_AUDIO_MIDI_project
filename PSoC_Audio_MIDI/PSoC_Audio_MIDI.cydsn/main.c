...
void setWaveDACnotes(uint8 n1, uint8 n2){
  freqClockDivCounter = 250000 / intFrequency[n1];
  Clock_1_SetDividerValue(freqClockDivCounter);
  freqClockDivCounter = 250000 / intFrequency[n2];
    ...
    if (songPosition >= songLength || (songPosition == 0 && songRepeatCounter == 0)){ 
       ...
       switch(songRepeatCounter){
            case 0: WaveDAC8_1_Wave1Setup(sawtooth, 502);
                    WaveDAC8_2_Wave1Setup(sawtooth, 502);
                    break;
            ...
            case 4: WaveDAC8_1_Wave1Setup(sinewave, 502);
                    WaveDAC8_2_Wave1Setup(trianglewave, 502);
                    break;
            case 5: WaveDAC8_1_Wave1Setup(trianglewave, 502);
                    WaveDAC8_2_Wave1Setup(squarewave, 502);
                    break;
            case 6: WaveDAC8_1_Wave1Setup(sawtooth, 502);
                    WaveDAC8_2_Wave1Setup(squareXSine, 502);
                    break;       
            default:  break;
        }
        songRepeatCounter++;
        songDelayMs--;
        songDelayMs = songDelayMs < 1 ? 12 : songDelayMs;
        songRepeatCounter = songRepeatCounter > 6 ? 0 : songRepeatCounter;
    }
    else{
        bar = (songPosition) / 96 + 1;
        songPosition++;
        //weddingMarch(songPosition);
...
//    sprintf(transmitBuffer, "wavemode: %i old: %i new: %i \n\r", waveMode, oldYinput, yInput);             
//    UART_1_PutString(transmitBuffer);
    oldYinput = yInput;
}
...
//            UART_1_PutString(transmitBuffer);
            ...
            sprintf(transmitBuffer, "pitch[1]: %i \n\r", pitch[1]);             
//            UART_1_PutString(transmitBuffer);
        ...
        switch (mode){
            ...
            case 1:
                ...
                break;
            case 2:
                weddingMarchMendelssohnBartholdy();
                break;
            case 3:
                if(button1){
                    WaveDAC8_1_Start();

...
/*******************************************************************************
* Function Name: mode_clock_Start
********************************************************************************
*
* Summary:
...
void mode_clock_Start(void) 
{
    ...
    mode_clock_CLKEN |= mode_clock_CLKEN_MASK;
	mode_clock_CLKSTBY |= mode_clock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: mode_clock_Stop
********************************************************************************
*
* Summary:
...
void mode_clock_Stop(void) 
{
    ...
    mode_clock_CLKEN &= (uint8)(~mode_clock_CLKEN_MASK);
	mode_clock_CLKSTBY &= (uint8)(~mode_clock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: mode_clock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((mode_clock_CLKEN & mode_clock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = mode_clock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = mode_clock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* mode_clock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(mode_clock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(mode_clock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: mode_clock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: mode_clock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = mode_clock_GetSourceRegister();
    uint16 oldDivider = mode_clock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = mode_clock_CLKEN & mode_clock_CLKEN_MASK;
        ...
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((mode_clock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
...
/*******************************************************************************
* Function Name: mode_clock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(mode_clock_DIV_PTR);
}


/*******************************************************************************
* Function Name: mode_clock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    mode_clock_MOD_SRC |= modeBitMask & (uint8)mode_clock_MODE_MASK;
}


/*******************************************************************************
* Function Name: mode_clock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    mode_clock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(mode_clock_MODE_MASK));
}


/*******************************************************************************
* Function Name: mode_clock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return mode_clock_MOD_SRC & (uint8)(mode_clock_MODE_MASK);
}


/*******************************************************************************
* Function Name: mode_clock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = mode_clock_GetDividerRegister();
    uint8 oldSrc = mode_clock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        mode_clock_MOD_SRC |= CYCLK_SSS;
        mode_clock_MOD_SRC =
            (mode_clock_MOD_SRC & (uint8)(~mode_clock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: mode_clock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return mode_clock_MOD_SRC & mode_clock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: mode_clock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    mode_clock_PHASE = clkPhase & mode_clock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: mode_clock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return mode_clock_PHASE & mode_clock_PHASE_MASK;
}
